// ============================================================
// VECTORA > GitHub Sync
// File: vectora_vSync.gs
//
// Syncs current VECTORA source to github.com/vectora-systems/vectora
// using the GitHub Git Data API (atomic multi-file commit).
//
// SETUP (one-time):
//   1. Extensions > Apps Script > Project Settings > Script Properties
//   2. Add property: GITHUB_TOKEN  → your PAT (Contents: read+write)
...
var VXSYNC_GH_OWNER = 'vectora-systems';
var VXSYNC_GH_REPO  = 'vectora';
var VXSYNC_GH_DIR   = 'src';
...
function diagnoseSyncSetup() {
  ...
  var repo = VxSync_ghGet_(ghToken, '/repos/' + VXSYNC_GH_OWNER + '/' + VXSYNC_GH_REPO);
  if (!repo || !repo.name) {
    Logger.log('❌ Repo access failed. Verify token has Contents: read+write scope for vectora-systems/vectora.');
    return;
  }
  Logger.log('Repo access  : ✅ ' + repo.full_name + ' (' + (repo.private ? 'private' : 'public') + ')');

  // 3. Branch exists
  var ref = VxSync_ghGet_(ghToken, '/repos/' + VXSYNC_GH_OWNER + '/' + VXSYNC_GH_REPO + '/git/refs/heads/' + branch);
  if (!ref || !ref.object) {
  ...
  var files = VxSync_fetchScriptFiles_(scriptId, oauthToken);
  if (!files) {
  ...
  files.forEach(function(f) {
    ...
    Logger.log('  ' + VXSYNC_GH_DIR + '/' + f.name + ext + '  (' + (f.source ? f.source.length : 0) + ' chars)');
  });
...
function syncVectoraToGithub() {
  ...
  var branch = props.getProperty('GITHUB_BRANCH') || 'main';
  Logger.log('Target: ' + VXSYNC_GH_OWNER + '/' + VXSYNC_GH_REPO + ' @ ' + branch);
  ...
  files.forEach(function(file) {
    ...
    fileMap[VXSYNC_GH_DIR + '/' + file.name + ext] = file.source;
  });

  var paths     = Object.keys(fileMap);
  var fileCount = paths.length;
  Logger.log('Preparing to commit ' + fileCount + ' files to ' + VXSYNC_GH_DIR + '/');
  ...
  var headCommit = VxSync_ghGet_(ghToken, '/repos/' + VXSYNC_GH_OWNER + '/' + VXSYNC_GH_REPO + '/git/commits/' + latestCommitSha);
  ...
  for (var i = 0; i < paths.length; i++) {
    ...
    var blob    = VxSync_ghPost_(ghToken, '/repos/' + VXSYNC_GH_OWNER + '/' + VXSYNC_GH_REPO + '/git/blobs', {
      content:  content,
      encoding: 'utf-8'
    });
  ...
  var newTree = VxSync_ghPost_(ghToken, '/repos/' + VXSYNC_GH_OWNER + '/' + VXSYNC_GH_REPO + '/git/trees', {
  ...
  var newCommit = VxSync_ghPost_(ghToken, '/repos/' + VXSYNC_GH_OWNER + '/' + VXSYNC_GH_REPO + '/git/commits', {
  ...
  var updated = VxSync_ghPatch_(ghToken, '/repos/' + VXSYNC_GH_OWNER + '/' + VXSYNC_GH_REPO + '/git/refs/heads/' + branch, {
    sha: newCommit.sha
  });
  if (!updated || !updated.object) { Logger.log('❌ Failed to update branch ref.'); return; }

  Logger.log('');
  Logger.log('✅ Synced ' + fileCount + ' files to ' + VXSYNC_GH_OWNER + '/' + VXSYNC_GH_REPO + ' @ ' + branch);
  Logger.log('🔗 ' + (newCommit.html_url || newCommit.sha));
}

// ---- Internal helpers ----

function VxSync_fetchScriptFiles_(scriptId, oauthToken) {
...
function VxSync_ghGet_(token, path) {
  var resp = UrlFetchApp.fetch('https://api.github.com' + path, {
    method:             'get',
    headers:            VxSync_ghHeaders_(token),
    muteHttpExceptions: true
  });
...
function VxSync_ghPost_(token, path, body) {
  var resp = UrlFetchApp.fetch('https://api.github.com' + path, {
    method:             'post',
    headers:            VxSync_ghHeaders_(token),
...
function VxSync_ghPatch_(token, path, body) {
  var resp = UrlFetchApp.fetch('https://api.github.com' + path, {
    method:             'patch',
    headers:            VxSync_ghHeaders_(token),
    payload:            JSON.stringify(body),
...
function VxSync_ghHeaders_(token) {
  return {
